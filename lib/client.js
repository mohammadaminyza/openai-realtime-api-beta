...
export class RealtimeClient extends RealtimeEventHandler {
    ...
    if (content.length) {
      for (const c of content) {
        ...
        if (c.type === 'input_video') {
          if (c.video instanceof ArrayBuffer || c.video instanceof Uint8Array) {
            c.video = RealtimeUtils.arrayBufferToBase64(c.video);
          }
        }
      }
      this.realtime.send('conversation.item.create', {
  ...
  createResponse() {
    if (this.getTurnDetectionType() === null) {
      if (this.inputAudioBuffer.byteLength > 0) {
      ...
      if (this.inputVideoBuffer.byteLength > 0) {
        ...
        this.conversation.queueInputVideo(this.inputVideoBuffer);
        this.inputVideoBuffer = new Uint8Array(0);
      }
    }
    this.realtime.send('response.create');
