...
export class RealtimeClient extends RealtimeEventHandler {
   ...
   */
  constructor({ url, token, turnDetection, debug } = {}) {
    super();
  ...
  _resetConfig() {
    ...
    this.inputVideoBuffer = new Uint8Array(0);
    return true;
  }
  ...
  _addAPIEventHandlers() {
    ...
    const handlerWithDispatch = (event, ...args) => {
      ...
      if (item) {
        ...
        //        can fire before `item.added`, resulting in empty item.
    ...
    this.realtime.on('server.response.output_item.created', handler);
    this.realtime.on('server.response.content_part.added', handler);
    this.realtime.on('server.response.content_part.done', handler);
    this.realtime.on('server.response.output_text.done', handler);
    this.realtime.on('server.response.output_audio.done', handler);
    this.realtime.on('server.response.output_audio_transcript.done', handler);
    this.realtime.on('server.response.function_call_arguments.done', handler);
    this.realtime.on('server.response.done', handler);
  ...
  updateVision(content) {
    this.realtime.send('vision.update', content);
    return true;
  }
   ...
   */
  updateSession({ tools = void 0, tool_choice = void 0 } = {}) {
   ...
   * @returns {boolean}
   */
  sendAssistantMessageContent(text) {
   ...
   * @returns {true}
   */
  appendInputVideo(arrayBuffer) {
    if (arrayBuffer.byteLength > 0) {
      this.realtime.send('input_video_buffer.append', {
        video: RealtimeUtils.arrayBufferToBase64(arrayBuffer),
      });
      this.inputVideoBuffer = RealtimeUtils.mergeUint8Arrays(
        this.inputVideoBuffer,
        arrayBuffer,
      );
    }
    return true;
  ...
  createResponse() {
    ...
    if (this.inputVideoBuffer.byteLength > 0) {
      this.realtime.send('input_video_buffer.commit');
      this.inputVideoBuffer = new Uint8Array(0);
    }
    this.realtime.send('response.create');
    return true;
  ...
  cancelResponse(id, sampleCount = 0) {
    if (!id) {
      ...
      this.realtime.send('response.cancel');
      const audioIndex = item.content.findIndex((c) =>
        ['audio', 'output_audio'].includes(c.type),
      );
      if (audioIndex === -1) {
