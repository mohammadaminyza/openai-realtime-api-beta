...
export class RealtimeConversation {
  ...
  EventProcessors = {
    'conversation.item.created': (event) => {
      ...
      newItem.formatted.video = new Uint8Array(0);
      ...
      if (newItem.type === 'message') {
        if (newItem.role === 'user') {
          ...
          if (this.queuedInputVideo) {
            newItem.formatted.video = this.queuedInputVideo;
            this.queuedInputVideo = null;
          }
        } else {
   ...
   * @returns {Uint8Array}
   */
  queueInputVideo(inputVideo) {
    this.queuedInputVideo = inputVideo;
    return inputVideo;
  }
