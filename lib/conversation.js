...
export class RealtimeConversation {
  ...
  static TEXT_CONTENT_TYPES = ['text', 'input_text', 'output_text'];
  static AUDIO_CONTENT_TYPES = ['audio', 'output_audio'];

  EventProcessors = {
    'conversation.item.added': (event) => {
      const { item } = event;
      const newItem = this._registerItem(item);
      return { item: newItem, delta: null };
    },
    'conversation.item.done': (event) => {
      const { item } = event;
      if (!item) {
        throw new Error(`conversation.item.done: Missing "item"`);
      }
      let foundItem = this.itemLookup[item.id];
      if (!foundItem) {
        foundItem = this._registerItem(item);
      } else {
    ...
    'conversation.item.input_audio_transcription.completed': (event) => {
      ...
      if (!item) {
        // We can receive transcripts in VAD mode before item.added
    ...
    'response.output_item.added': (event) =>
      this._processOutputItemAdded(event),
    'response.output_item.created': (event) =>
      this._processOutputItemAdded(event),
    ...
    'response.content_part.done': (event) => {
      ...
      if (!item) {
        throw new Error(
          `response.content_part.done: Item "${item_id}" not found`,
        );
      }
      return { item, delta: null };
    },
    'response.output_audio_transcript.delta': (event) => {
      ...
      if (!item.content[content_index].transcript) {
    ...
    'response.output_audio_transcript.done': (event) => {
      ...
      if (!item) {
        throw new Error(
          `response.output_audio_transcript.done: Item "${item_id}" not found`,
        );
      }
      return { item, delta: null };
    },
    'response.output_audio.delta': (event) => {
      ...
      if (!item) {
        throw new Error(
          `response.output_audio.delta: Item "${item_id}" not found`,
        );
      }
    ...
    'response.output_audio.done': (event) => {
      ...
      if (!item) {
        throw new Error(
          `response.output_audio.done: Item "${item_id}" not found`,
        );
      }
      return { item, delta: null };
    },
    'response.output_text.delta': (event) => {
      ...
      if (!item) {
        throw new Error(
          `response.output_text.delta: Item "${item_id}" not found`,
        );
      }
      if (!item.content[content_index].text) {
    ...
    'response.output_text.done': (event) => {
      ...
      if (!item) {
        throw new Error(
          `response.output_text.done: Item "${item_id}" not found`,
        );
      }
      return { item, delta: null };
    },
  ...
  _registerItem(item) {
    ...
    if (newItem.content) {
      const textContent = newItem.content.filter((c) =>
        RealtimeConversation.TEXT_CONTENT_TYPES.includes(c.type),
      );
  ...
  _processOutputItemAdded(event) {
    ...
    if (!response.output.includes(item.id)) {
